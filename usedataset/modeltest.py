...
def transfer_data():
    ...
    for iii in range(2223, total_slice_num):
        slice_file = open(path + sliceDir[iii], 'r')
        ...
        for one_slice in slices:
            ...
            test_label[true_data_len] = torch.Tensor(input_label[iii])
            true_data_len += 1
...
net = Net().to(device)
net.load_state_dict(torch.load('2 0+coon-meanfreq.pt'))
net.eval()
def test():
    global test_data
    global test_label
