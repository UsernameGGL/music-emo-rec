    ...
    for ii in range(epoch_num):
        ...
        for sliceFNama in sliceDir:
            ...
            for one_slice in slices:
                ...
                time_data = (time_data - min(time_data)) / (max(time_data) - min(time_data))
                freq_data = np.array(freq_data)
                freq_data = (freq_data - min(freq_data)) / (max(freq_data) - min(freq_data))
...
def train():
    ...
    torch.save(net.state_dict(), '6-5+ininorm-epo.pt')
